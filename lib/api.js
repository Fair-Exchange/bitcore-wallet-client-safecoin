...
// var BitcoreBch = require('bitcore-lib-cash');
// var BitcoreBtcP = require('bitcore-lib').PrivateKey;
// var BitcoreBchP = require('bitcore-lib-cash').PrivateKey;
// var BitcoreSafe = require('bitcore-lib-safe');
/*  var Bitcore_ = {
  btc: BitcoreBtc,
  bch: require('bitcore-lib-cash'),
  safe: require('bitcore-lib-safe')
}; */ 
...
// var BASE_URL = 'http://192.168.91.206:3232/bws/api';
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testLiveKeys(coin) {
    ...
    if (coin && coin == 'btc') {
      ...
      BitcoreBtc = require('bitcore-lib-safe');
    } else if (coin && coin == 'btcz') {
      BitcoreBtc = require('bitcore-lib-btcz');
    } else if (coin && coin == 'zcl') {
      BitcoreBtc = require('bitcore-lib-zcl');
    } else if (coin && coin == 'anon') {
      BitcoreBtc = require('bitcore-lib-anon');
    } else if (coin && coin == 'zel') {
      BitcoreBtc = require('bitcore-lib-zel');
    } else if (coin && coin == 'rvn') {
      BitcoreBtc = require('bitcore-lib-rvn');
    } else if (coin && coin == 'ltc') {
      BitcoreBtc = require('bitcore-lib-ltc');
    } else {
      debugger;
    }
    var words;
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, coin, cb) {
  ...
  if (coin == 'btc') {
    ...
    CorePrvK = require('bitcore-lib-safe').PrivateKey;
  } else if (coin == 'btcz') {
    CorePrvK = require('bitcore-lib-btcz').PrivateKey;
  } else if (coin == 'zcl') {
    CorePrvK = require('bitcore-lib-zcl').PrivateKey;
  } else if (coin == 'anon') {
    CorePrvK = require('bitcore-lib-anon').PrivateKey;
  } else if (coin == 'zel') {
    CorePrvK = require('bitcore-lib-zel').PrivateKey;
  } else if (coin == 'rvn') {
    CorePrvK = require('bitcore-lib-rvn').PrivateKey;
  } else if (coin == 'ltc') {
    CorePrvK = require('bitcore-lib-ltc').PrivateKey;
  }
...
//  var B; // = Bitcore_[coin];
  ...
  if (coin == 'btc') {
    ...
    CorePrvK = require('bitcore-lib-safe').PrivateKey;
    CoreAddr = require('bitcore-lib-safe').Address;
    CoreTr = require('bitcore-lib-safe').Transaction;
  } else if (coin == 'btcz') {
    CorePrvK = require('bitcore-lib-btcz').PrivateKey;
    CoreAddr = require('bitcore-lib-btcz').Address;
    CoreTr = require('bitcore-lib-btcz').Transaction;
  } else if (coin == 'zcl') {
    CorePrvK = require('bitcore-lib-zcl').PrivateKey;
    CoreAddr = require('bitcore-lib-zcl').Address;
    CoreTr = require('bitcore-lib-zcl').Transaction;
  } else if (coin == 'anon') {
    CorePrvK = require('bitcore-lib-anon').PrivateKey;
    CoreAddr = require('bitcore-lib-anon').Address;
    CoreTr = require('bitcore-lib-anon').Transaction;
  } else if (coin == 'zel') {
    CorePrvK = require('bitcore-lib-zel').PrivateKey;
    ...
    CorePrvK = require('bitcore-lib-rvn').PrivateKey;
    CoreAddr = require('bitcore-lib-rvn').Address;
    CoreTr = require('bitcore-lib-rvn').Transaction;
  } else if (coin == 'ltc') {
    CorePrvK = require('bitcore-lib-ltc').PrivateKey;
    CoreAddr = require('bitcore-lib-ltc').Address;
    CoreTr = require('bitcore-lib-ltc').Transaction;
  } else {
    debugger;
  }
...
//  var bitcore; // = Bitcore_[txp.coin];
  ...
  if (txp.coin == 'btc') {
    ...
    CoreHDPub = require('bitcore-lib-safe').HDPublicKey;
    CoreCrSig = require('bitcore-lib-safe').crypto.Signature; 
  } else if (txp.coin == 'btcz') {
    CoreHDPub = require('bitcore-lib-btcz').HDPublicKey;
    CoreCrSig = require('bitcore-lib-btcz').crypto.Signature; 
  } else if (txp.coin == 'zcl') {
    CoreHDPub = require('bitcore-lib-zcl').HDPublicKey;
    CoreCrSig = require('bitcore-lib-zcl').crypto.Signature; 
  } else if (txp.coin == 'anon') {
    CoreHDPub = require('bitcore-lib-anon').HDPublicKey;
    CoreCrSig = require('bitcore-lib-anon').crypto.Signature; 
    ...
    CoreHDPub = require('bitcore-lib-rvn').HDPublicKey;
    CoreCrSig = require('bitcore-lib-rvn').crypto.Signature; 
  } else if (txp.coin == 'ltc') {
    CoreHDPub = require('bitcore-lib-ltc').HDPublicKey;
    CoreCrSig = require('bitcore-lib-ltc').crypto.Signature; 
  }
  var i = 0,
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['btc', 'bch', 'safe', 'btcz', 'zcl', 'anon', 'zel', 'rvn', 'ltc'], coin));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'safe';
  if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'zcl', 'anon', 'zel', 'rvn', 'ltc'], coin)) return cb(new Error('Invalid coin11'));
...
API.prototype.joinWallet = function(secret, copayerName, opts, cb) {
  ...
  var coin = opts.coin || 'safe';
  if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'zcl', 'anon', 'zel', 'rvn', 'ltc'], coin)) return cb(new Error('Invalid coin22'));
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'zcl', 'anon', 'zel', 'rvn', 'ltc'], opts.coin)) return cb(new Error('Invalid coin33'));
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  var coin = opts.coin || 'safe';
  if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'zcl', 'anon', 'zel', 'rvn', 'ltc'], coin)) return cb(new Error('Invalid coin44'));
