...
var Mnemonic = require('bitcore-mnemonic-safecoin');
...
var BASE_URL = 'https://api.safecoin.org/bwss/api';
var undf;
/**
 ...
 */
API._signRequest = function(method, url, args, privKey, coin) {
  if (!coin && !args.coin) debugger;
  var message = [method.toLowerCase(), url, JSON.stringify(args)].join('|');
  return Utils.signMessage(message, privKey, coin ? coin : args.coin);
};
...
//    var xpriv = Mnemonic(words, undf, 'btc');
    var xpriv = Mnemonic(words, undf, 'btc').toHDPrivateKey();
    xpriv.coin = 'btc';
//    xpriv.coin = xpriv.toHDPrivateKey();
    var BitcoreBtc = require('bitcore-lib');
    ...
    var xpub = BitcoreBtc.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys(coin) {
    var BitcoreBtc;
    if (coin && coin == 'btc') {
      BitcoreBtc = require('bitcore-lib');
    } else if (coin && coin == 'bch') {
      BitcoreBtc = require('bitcore-lib');
    } else if (coin && coin == 'safe') {
      BitcoreBtc = require('bitcore-lib-safecoin');
    } else if (coin && coin == 'btcz') {
      BitcoreBtc = require('bitcore-lib-btcz-mini');
    } else if (coin && coin == 'zcl') {
      BitcoreBtc = require('bitcore-lib-zcl-mini');
    } else if (coin && coin == 'rito') {
      BitcoreBtc = require('bitcore-lib-rito-mini');
    } else if (coin && coin == 'anon') {
      BitcoreBtc = require('bitcore-lib-anon-mini');
    } else if (coin && coin == 'zel') {
      BitcoreBtc = require('bitcore-lib-zel');
    } else if (coin && coin == 'zen') {
      BitcoreBtc = require('bitcore-lib-zen-mini');
    } else if (coin && coin == 'rvn') {
      BitcoreBtc = require('bitcore-lib-rvn-mini');
    } else if (coin && coin == 'ltc') {
      BitcoreBtc = require('bitcore-lib-ltc-mini');
    } else {
      debugger;
    }
    var words;
    ...
    if (words && (!c.mnemonicHasPassphrase || opts.passphrase)) {
      var m = new Mnemonic(words, undf, coin);
      m.coin = coin;
      xpriv = m.toHDPrivateKey(opts.passphrase, c.network);
    }
    if (!xpriv) {
      xpriv = new BitcoreBtc.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath(coin));
    var xpub = new BitcoreBtc.HDPublicKey(c.xPubKey);
  ...
  var liveOk = (c.canSign() && !c.isPrivKeyEncrypted()) ? testLiveKeys(c.coin) : true;
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new BitcoreBtc.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = BitcoreBtc.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = BitcoreBtc.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, coin, cb) {
  ...
  var CorePrvK;
  if (coin == 'btc') {
    CorePrvK = require('bitcore-lib').PrivateKey;
  } else if (coin == 'bch') {
    CorePrvK = require('bitcore-lib-cash').PrivateKey; // BitcoreBchP;
  } else if (coin == 'safe') {
    CorePrvK = require('bitcore-lib-safecoin').PrivateKey;
  } else if (coin == 'btcz') {
    CorePrvK = require('bitcore-lib-btcz-mini').PrivateKey;
  } else if (coin == 'zcl') {
    CorePrvK = require('bitcore-lib-zcl-mini').PrivateKey;
  } else if (coin == 'rito') {
    CorePrvK = require('bitcore-lib-rito-mini').PrivateKey;
  } else if (coin == 'anon') {
    CorePrvK = require('bitcore-lib-anon-mini').PrivateKey;
  } else if (coin == 'zel') {
    CorePrvK = require('bitcore-lib-zel').PrivateKey;
  } else if (coin == 'zen') {
    CorePrvK = require('bitcore-lib-zen-mini').PrivateKey;
  } else if (coin == 'rvn') {
    CorePrvK = require('bitcore-lib-rvn-mini').PrivateKey;
  } else if (coin == 'ltc') {
    CorePrvK = require('bitcore-lib-ltc-mini').PrivateKey;
  }
 
//  var privateKey = new B.PrivateKey(privateKey);
  var privateKey = new CorePrvK(privateKey);
...
//  var B; // = Bitcore_[coin];
  ...
  if (coin == 'btc') {
    ...
    CorePrvK = require('bitcore-lib-cash').PrivateKey;
    ...
    CoreTr = require('bitcore-lib-safecoin').Transaction;
  } else if (coin == 'btcz') {
    CorePrvK = require('bitcore-lib-btcz-mini').PrivateKey;
    ...
    CoreTr = require('bitcore-lib-rito-mini').Transaction;
  } else if (coin == 'anon') {
    CorePrvK = require('bitcore-lib-anon-mini').PrivateKey;
    ...
    CorePrvK = require('bitcore-lib-zel').PrivateKey;
    CoreAddr = require('bitcore-lib-zel').Address;
    CoreTr = require('bitcore-lib-zel').Transaction;
  } else if (coin == 'zen') {
    CorePrvK = require('bitcore-lib-zen-mini').PrivateKey;
    ...
    CoreTr = require('bitcore-lib-ltc-mini').Transaction;
  } else {
    debugger;
  }

  var privateKey = CorePrvK(privateKey);
  var address = privateKey.publicKey.toAddress();
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = CoreAddr.fromString(destinationAddress);
  ...
  if (self.credentials) {
    ...
    if (useSession && self.session) {
      ...
      if (key) {
        ...
        reqSignature = API._signRequest(method, url, args, key, this.coin || this.credentials.coin);
      }
...
API._buildSecret = function(walletId, walletPrivKey, coin, network) {
  ...
  if (_.isString(walletPrivKey)) {
    walletPrivKey = BitcoreBtc.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new BitcoreBtc.encoding.Base58(widHex).toString();
  return _.padEnd(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == 'testnet' ? 'T' : 'L') + coin;
};

API.parseSecret = function(secret, coiN) {
  ...
  try {
    ...
    var widHex = BitcoreBtc.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = BitcoreBtc.PrivateKey.fromString(secretSplit[1]);
    ...
    if (coin == coiN) {
      ...
      debugger;
      throw new Error('Invalid secret coin');
    }
  } catch (ex) {
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var derived = {};
  var BitcoreBtc;
  if (txp.coin && txp.coin == 'btc') {
    BitcoreBtc = require('bitcore-lib');
  } else if (txp.coin && txp.coin == 'bch') {
    BitcoreBtc = require('bitcore-lib');
  } else if (txp.coin && txp.coin == 'safe') {
    BitcoreBtc = require('bitcore-lib-safecoin');
  } else if (txp.coin && txp.coin == 'btcz') {
    BitcoreBtc = require('bitcore-lib-btcz-mini');
  } else if (txp.coin && txp.coin == 'zcl') {
    BitcoreBtc = require('bitcore-lib-zcl-mini');
  } else if (txp.coin && txp.coin == 'rito') {
    BitcoreBtc = require('bitcore-lib-rito-mini');
  } else if (txp.coin && txp.coin == 'anon') {
    BitcoreBtc = require('bitcore-lib-anon-mini');
  } else if (txp.coin && txp.coin == 'zel') {
    BitcoreBtc = require('bitcore-lib-zel');
  } else if (txp.coin && txp.coin == 'zen') {
    BitcoreBtc = require('bitcore-lib-zen-mini');
  } else if (txp.coin && txp.coin == 'rvn') {
    BitcoreBtc = require('bitcore-lib-rvn-mini');
  } else if (txp.coin && txp.coin == 'ltc') {
    BitcoreBtc = require('bitcore-lib-ltc-mini');
  } else {
    debugger;
  }
  var xpriv = new BitcoreBtc.HDPrivateKey(derivedXPrivKey);
...
//  var bitcore; // = Bitcore_[txp.coin];
  ...
  var CoreCrSig; 
  if (txp.coin == 'btc') {
    CoreHDPub = require('bitcore-lib').HDPublicKey;
    CoreCrSig = require('bitcore-lib').crypto.Signature; 
  } else if (txp.coin == 'bch') {
    CoreHDPub = require('bitcore-lib-cash').HDPublicKey;
    CoreCrSig = require('bitcore-lib-cash').crypto.Signature; 
  } else if (txp.coin == 'safe') {
    CoreHDPub = require('bitcore-lib-safecoin').HDPublicKey;
    CoreCrSig = require('bitcore-lib-safecoin').crypto.Signature; 
  } else if (txp.coin == 'btcz') {
    CoreHDPub = require('bitcore-lib-btcz-mini').HDPublicKey;
    CoreCrSig = require('bitcore-lib-btcz-mini').crypto.Signature; 
  } else if (txp.coin == 'zcl') {
    CoreHDPub = require('bitcore-lib-zcl-mini').HDPublicKey;
    CoreCrSig = require('bitcore-lib-zcl-mini').crypto.Signature; 
  } else if (txp.coin == 'rito') {
    CoreHDPub = require('bitcore-lib-rito-mini').HDPublicKey;
    CoreCrSig = require('bitcore-lib-rito-mini').crypto.Signature; 
  } else if (txp.coin == 'anon') {
    CoreHDPub = require('bitcore-lib-anon-mini').HDPublicKey;
    CoreCrSig = require('bitcore-lib-anon-mini').crypto.Signature; 
  } else if (txp.coin == 'zel') {
    CoreHDPub = require('bitcore-lib-zel').HDPublicKey;
    CoreCrSig = require('bitcore-lib-zel').crypto.Signature; 
  } else if (txp.coin == 'zen') {
    CoreHDPub = require('bitcore-lib-zen-mini').HDPublicKey;
    CoreCrSig = require('bitcore-lib-zen-mini').crypto.Signature; 
  } else if (txp.coin == 'rvn') {
    CoreHDPub = require('bitcore-lib-rvn-mini').HDPublicKey;
    CoreCrSig = require('bitcore-lib-rvn-mini').crypto.Signature; 
  } else if (txp.coin == 'ltc') {
    CoreHDPub = require('bitcore-lib-ltc-mini').HDPublicKey;
    CoreCrSig = require('bitcore-lib-ltc-mini').crypto.Signature; 
  }
  var i = 0,
    x = new CoreHDPub(xpub);

  _.each(signatures, function(signatureHex) {
    var input = txp.inputs[i];
    try {
      var signature = CoreCrSig.fromString(signatureHex);
      var pub = x.deriveChild(txp.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: CoreCrSig.SIGHASH_ALL | CoreCrSig.SIGHASH_FORKID,
        publicKey: pub,
      }
      ;
 ...
 *
 * @param {string} coin - 'safe' (default) or 'bch'
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['btc', 'bch', 'safe', 'btcz', 'rito', 'zcl', 'anon', 'zel', 'zen', 'rvn', 'ltc'], coin));
 ...
 * @param {string} opts.coin[='safe'] - The coin for this wallet (btc, bch).
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  var self = this;
  var BitcoreBtc;
  if (opts.coin && opts.coin == 'btc') {
    BitcoreBtc = require('bitcore-lib');
  } else if (opts.coin && opts.coin == 'bch') {
    BitcoreBtc = require('bitcore-lib');
  } else if (opts.coin && opts.coin == 'safe') {
    BitcoreBtc = require('bitcore-lib-safecoin');
  } else if (opts.coin && opts.coin == 'btcz') {
    BitcoreBtc = require('bitcore-lib-btcz-mini');
  } else if (opts.coin && opts.coin == 'zcl') {
    BitcoreBtc = require('bitcore-lib-zcl-mini');
  } else if (opts.coin && opts.coin == 'rito') {
    BitcoreBtc = require('bitcore-lib-rito-mini');
  } else if (opts.coin && opts.coin == 'anon') {
    BitcoreBtc = require('bitcore-lib-anon-mini');
  } else if (opts.coin && opts.coin == 'zel') {
    BitcoreBtc = require('bitcore-lib-zel');
  } else if (opts.coin && opts.coin == 'zen') {
    BitcoreBtc = require('bitcore-lib-zen-mini');
  } else if (opts.coin && opts.coin == 'rvn') {
    BitcoreBtc = require('bitcore-lib-rvn-mini');
  } else if (opts.coin && opts.coin == 'ltc') {
    BitcoreBtc = require('bitcore-lib-ltc-mini');
  } else {
    debugger;
  }
  ...
  var coin = opts.coin || 'safe';
  if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'rito', 'zcl', 'anon', 'zel', 'zen', 'rvn', 'ltc'], coin)) return cb(new Error('Invalid coin11'));
  ...
  var walletPrivKey = opts.walletPrivKey || new BitcoreBtc.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new BitcoreBtc.PrivateKey(walletPrivKey)).toPublicKey().toString(),
    coin: coin,
    network: network,
 ...
 * @param {String} copayerName
 * @param {Object} opts
 * @param {string} opts.coin[='safe'] - The expected coin for this wallet (btc, bch).
...
API.prototype.joinWallet = function(secret, copayerName, opts, cb) {
  ...
  var coin = opts.coin || 'safe';
  if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'rito', 'zcl', 'anon', 'zel', 'zen', 'rvn', 'ltc'], coin)) return cb(new Error('Invalid coin22'));
...
API.prototype.recreateWallet = function(cb) {
  ...
  var self = this;
  var BitcoreBtc;
  if (self.credentials.coin && self.credentials.coin == 'btc') {
    BitcoreBtc = require('bitcore-lib');
  } else if (self.credentials.coin && self.credentials.coin == 'bch') {
    BitcoreBtc = require('bitcore-lib');
  } else if (self.credentials.coin && self.credentials.coin == 'safe') {
    BitcoreBtc = require('bitcore-lib-safecoin');
  } else if (self.credentials.coin && self.credentials.coin == 'btcz') {
    BitcoreBtc = require('bitcore-lib-btcz-mini');
  } else if (self.credentials.coin && self.credentials.coin == 'zcl') {
    BitcoreBtc = require('bitcore-lib-zcl-mini');
  } else if (self.credentials.coin && self.credentials.coin == 'rito') {
    BitcoreBtc = require('bitcore-lib-rito-mini');
  } else if (self.credentials.coin && self.credentials.coin == 'anon') {
    BitcoreBtc = require('bitcore-lib-anon-mini');
  } else if (self.credentials.coin && self.credentials.coin == 'zel') {
    BitcoreBtc = require('bitcore-lib-zel');
  } else if (self.credentials.coin && self.credentials.coin == 'zen') {
    BitcoreBtc = require('bitcore-lib-zen-mini');
  } else if (self.credentials.coin && self.credentials.coin == 'rvn') {
    BitcoreBtc = require('bitcore-lib-rvn-mini');
  } else if (self.credentials.coin && self.credentials.coin == 'ltc') {
    BitcoreBtc = require('bitcore-lib-ltc-mini');
  } else {
    debugger;
  }
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = BitcoreBtc.PrivateKey.fromString(c.walletPrivKey);
...
API.prototype.fetchPayPro = function(opts, cb) {
  ...
  PayPro.get({
    ...
    coin: this.credentials.coin || 'safe',
  }, function(err, paypro) { 
    if (err)
      return cb(err);
...
// debugger;
  ...
  var args = {
    proposalSignature: Utils.signMessage(hash, self.credentials.requestPrivKey, opts.txp.coin)
  };
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'rito', 'zcl', 'anon', 'zel', 'zen', 'rvn', 'ltc'], opts.coin)) return cb(new Error('Invalid coin33'));
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  var coin = opts.coin || 'safe';
  if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'rito', 'zcl', 'anon', 'zel', 'zen', 'rvn', 'ltc'], coin)) return cb(new Error('Invalid coin44'));
...
API.prototype.addAccess = function(opts, cb) {
  ...
  opts = opts || {};
  if (!opts.coin && this.credentials.coin) opts.coin = this.credentials.coin;
  var BitcoreBtc;
  if (opts.coin && opts.coin == 'btc') {
    BitcoreBtc = require('bitcore-lib');
  } else if (opts.coin && opts.coin == 'bch') {
  ...
  var reqPrivKey = new BitcoreBtc.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new BitcoreBtc.HDPrivateKey(this.credentials.xPrivKey)
    .deriveChild(this.credentials.getBaseAddressDerivationPath(opts.coin));
  var sig = Utils.signRequestPubKey(requestPubKey, xPriv, opts.coin);
