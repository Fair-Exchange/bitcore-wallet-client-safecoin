...
function _checkCoin(coin) {
  if (!_.includes(['btc', 'bch', 'safe', 'btcz', 'rito', 'zcl', 'anon', 'zel', 'zen', 'rvn', 'ltc'], coin)) throw new Error('Invalid coin66');
};
...
Credentials.create = function(coin, network) {
  ...
  var Bitcore;
  if (coin && coin == 'btc') {
    Bitcore = require('bitcore-lib').HDPrivateKey;
  } else if (coin && coin == 'bch') {
    Bitcore = require('bitcore-lib').HDPrivateKey;
  } else if (coin && coin == 'safe') {
    Bitcore = require('bitcore-lib-safe-mini').HDPrivateKey;
  } else if (coin && coin == 'btcz') {
    Bitcore = require('bitcore-lib-btcz-mini').HDPrivateKey;
  } else if (coin && coin == 'zcl') {
    Bitcore = require('bitcore-lib-zcl-mini').HDPrivateKey;
  } else if (coin && coin == 'rito') {
    Bitcore = require('bitcore-lib-rito-mini').HDPrivateKey;
  } else if (coin && coin == 'anon') {
    Bitcore = require('bitcore-lib-anon-mini').HDPrivateKey;
  } else if (coin && coin == 'zel') {
    Bitcore = require('bitcore-lib-zel-mini').HDPrivateKey;
  } else if (coin && coin == 'zen') {
    Bitcore = require('bitcore-lib-zen-mini').HDPrivateKey;
  } else if (coin && coin == 'rvn') {
    Bitcore = require('bitcore-lib-rvn-mini').HDPrivateKey;
  } else if (coin && coin == 'ltc') {
    Bitcore = require('bitcore-lib-ltc-mini').HDPrivateKey;
  } else {
    debugger;
  }
  ...
  x.coin = coin;
  x.network = network;
  x.xPrivKey = (new Bitcore(network)).toString();
...
Credentials.createWithMnemonic = function(coin, network, passphrase, language, account, opts) {
  ...
  opts = opts || {};
  var m = new Mnemonic(wordsForLang[language], undf, coin);
...
Credentials.fromExtendedPublicKey = function(coin, xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  if (coin == 'btc') {
    ...
    Bitcore = require('bitcore-lib-rito-mini');
  } else if (coin == 'anon') {
...
Credentials.prototype._hashFromEntropy = function(prefix, length) {
  $.checkState(prefix);
  var Bitcore;
  if (this.coin && this.coin == 'btc') {
    Bitcore = require('bitcore-lib');
  } else if (this.coin && this.coin == 'bch') {
    Bitcore = require('bitcore-lib');
  } else if (this.coin && this.coin == 'safe') {
    Bitcore = require('bitcore-lib-safe-mini');
  } else if (this.coin && this.coin == 'btcz') {
    Bitcore = require('bitcore-lib-btcz-mini');
  } else if (this.coin && this.coin == 'zcl') {
    Bitcore = require('bitcore-lib-zcl-mini');
  } else if (this.coin && this.coin == 'rito') {
    Bitcore = require('bitcore-lib-rito-mini');
  } else if (this.coin && this.coin == 'anon') {
    Bitcore = require('bitcore-lib-anon-mini');
  } else if (this.coin && this.coin == 'zel') {
    Bitcore = require('bitcore-lib-zel-mini');
  } else if (this.coin && this.coin == 'zen') {
    Bitcore = require('bitcore-lib-zen-mini');
  } else if (this.coin && this.coin == 'rvn') {
    Bitcore = require('bitcore-lib-rvn-mini');
  } else if (this.coin && this.coin == 'ltc') {
    Bitcore = require('bitcore-lib-ltc-mini');
  } else {
    debugger;
  }
...
Credentials.prototype._expand = function() {
  ...
  if (this.coin && this.coin == 'btc') {
    Bitcore = require('bitcore-lib');
    DervPath = Constants.PATHS.REQUEST_KEY_BTC; 
  } else if (this.coin && this.coin == 'bch') {
    Bitcore = require('bitcore-lib');
    DervPath = Constants.PATHS.REQUEST_KEY_BCH; 
  } else if (this.coin && this.coin == 'safe') {
    Bitcore = require('bitcore-lib-safe-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_SAFE; 
  } else if (this.coin && this.coin == 'btcz') {
    Bitcore = require('bitcore-lib-btcz-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_BTCZ; 
  } else if (this.coin && this.coin == 'zcl') {
    Bitcore = require('bitcore-lib-zcl-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_ZCL; 
  } else if (this.coin && this.coin == 'rito') {
    Bitcore = require('bitcore-lib-rito-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_RITO; 
  } else if (this.coin && this.coin == 'anon') {
    Bitcore = require('bitcore-lib-anon-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_ANON; 
  } else if (this.coin && this.coin == 'zel') {
    Bitcore = require('bitcore-lib-zel-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_ZEL; 
  } else if (this.coin && this.coin == 'zen') {
    Bitcore = require('bitcore-lib-zen-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_ZEN; 
  } else if (this.coin && this.coin == 'rvn') {
    Bitcore = require('bitcore-lib-rvn-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_RVN; 
  } else if (this.coin && this.coin == 'ltc') {
    Bitcore = require('bitcore-lib-ltc-mini');
    DervPath = Constants.PATHS.REQUEST_KEY_LTC; 
  } else {
    debugger;
  }
  ...
  if (this.xPrivKey) {
    ...
    var derivedXPrivKey = deriveFn(this.getBaseAddressDerivationPath(this.coin));
  ...
  if (this.entropySource) {
    ...
    var requestDerivation = deriveFn(DervPath);
  ...
  this.personalEncryptingKey = this._hashFromEntropy('personalKey', 16, this.coin).toString('base64');
...
Credentials.prototype.getBaseAddressDerivationPath = function(coin) {
  var purpose;
  ...
  var cointype = (this.network == 'testnet' ? "1" :
      coin == 'ltc' ? "2" : 
      coin == 'zel' ? "19167" : 
      coin == 'zen' ? "121" : 
      coin == 'zcl' ? "147" :
  ...
  return "m/" + purpose + "'/" + cointype + "'/" + this.account + "'";
};
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var path = this.getBaseAddressDerivationPath(this.coin);
...
Credentials.fromOldCopayWallet = function(w) {
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      var path = Constants.PATHS.REQUEST_KEY_BTC;
