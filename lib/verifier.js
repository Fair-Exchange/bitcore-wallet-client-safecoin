 ...
 * @returns {Boolean} true or false
 */
Verifier.checkCopayers = function(credentials, copayers, coin) {
  $.checkState(credentials.walletPrivKey);
  var CorePrivKeyFrStr;
  if (!coin) coin = credentials.coin;
  if (coin && coin == 'btc') {
    CorePrivKeyFrStr = require('bitcore-lib').PrivateKey.fromString;
  } else if (coin && coin == 'bch') {
    CorePrivKeyFrStr = require('bitcore-lib').PrivateKey.fromString;
  } else if (coin && coin == 'safe') {
    CorePrivKeyFrStr = require('bitcore-lib-safe-mini').PrivateKey.fromString;
  } else if (coin && coin == 'btcz') {
    CorePrivKeyFrStr = require('bitcore-lib-btcz-mini').PrivateKey.fromString;
  } else if (coin && coin == 'zcl') {
    CorePrivKeyFrStr = require('bitcore-lib-zcl-mini').PrivateKey.fromString;
  } else if (coin && coin == 'rito') {
    CorePrivKeyFrStr = require('bitcore-lib-rito-mini').PrivateKey.fromString;
  } else if (coin && coin == 'anon') {
    CorePrivKeyFrStr = require('bitcore-lib-anon-mini').PrivateKey.fromString;
  } else if (coin && coin == 'zel') {
    CorePrivKeyFrStr = require('bitcore-lib-zel-mini').PrivateKey.fromString;
  } else if (coin && coin == 'zen') {
    CorePrivKeyFrStr = require('bitcore-lib-zen-mini').PrivateKey.fromString;
  } else if (coin && coin == 'rvn') {
    CorePrivKeyFrStr = require('bitcore-lib-rvn-mini').PrivateKey.fromString;
  } else if (coin && coin == 'ltc') {
    CorePrivKeyFrStr = require('bitcore-lib-ltc-mini').PrivateKey.fromString;
  } else {
    debugger;
  }

  var walletPubKey = CorePrivKeyFrStr(credentials.walletPrivKey).toPublicKey().toString();
  ...
  _.each(copayers, function(copayer) {
    ...
    if (!(copayer.encryptedName || copayer.name) || !copayer.xPubKey || !copayer.requestPubKey || !copayer.signature) {
      ...
      if (!Utils.verifyMessage(hash, copayer.signature, walletPubKey, copayer.coin)) {
